...
const login = async (req, res, next) => {
  ...
  try {
    user = await authServices.getUserBy({ email }, '+password');
...
const signUp = async (req, res, next) => {
  ...
  try {
    let user;
    user = await authServices.getUserBy({ nickname });
    if (user) throw createError(400, 'Nickname already in use.');
    user = await authServices.getUserBy({ email });
  ...
  try {
    ...
    createdUser = await authServices.createUser(user);
  } catch (err) {
    return next(createError(500, 'Signing up failed, please try again later.'));
  }
